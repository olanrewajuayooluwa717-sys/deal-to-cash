    ...
    <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span
              className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                resolvedPreview.mode === 'brittle'
                  ? 'bg-red-100 text-red-800'
                  : 'bg-sky-100 text-sky-800'
              }`}
            >
              {resolvedPreview.mode === 'brittle' ? 'Zapier Mode' : 'Agent Mode'}
            </span>
          </div>
        ...
        <button
          type="button"
          onClick={syncToXero}
          disabled={syncing || resolvedPreview.mode === 'brittle'}
      ...
      {resolvedPreview.mode === 'brittle' && (
        <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-900">
          Switch to <strong>Agent Mode</strong> before syncing — brittle mappings would create bad invoices in Xero.
        </p>
      )}
